viewModel.appName = ko.observable('EACIIT App')
...
viewModel.isLoadingState = ko.observable(false)
...
//             opacity: 1
//         }, 'fast', function () {
//             viewModel.isLoadingState(false)
//         })
//     } else {
//         $('#loader').animate({
//             opacity: 0
...
viewModel.checkSidebarAccess = function () {
    viewModel.ajaxPostCallback('/main/auth/getuserinfo', {}, function (data) {
        if (data.IsImportant) {
            $('[href="/main/access/master"]').show()
        } else {
            $('[href="/main/access/master"]').remove()
        }
    })
}
